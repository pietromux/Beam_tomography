function [idx, neg_x, neg_y, weights, conv_param,A,target] = compare_images(phsp_i, phsp_f, target, pixelcal, display)
...
for j = 1:size(phsp_f,1)
    ix = round(phsp_f(j,1)/pixelcal+imsize(2)/2);
    iy = round(-phsp_f(j,3)/pixelcal+imsize(1)/2);
    if (ix<1 | ix>imsize(2) | iy<1 | iy>imsize(1))
       idx(j) = true ;
       weights(j) = 1;
    else
       idx(j) = (difference(iy,ix) > 0.5*sqrt(target(iy,ix)));
